...
def main():
    ...
    data_s, label_s = mindgap_utils.load_data_pavia(data_path_s, label_path_s)
    data_t, label_t = mindgap_utils.load_data_pavia(data_path_t, label_path_t)
    data_s, data_t = ILDA(data_s, data_t, cfg.pca_n, cfg.radius)

    cfg.inverse_lr_schedule = False
...
from mluda_model import DSANSS
import time
import utils
...
for iDataSet in range(nDataSet):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i in range(1,num_iter):
            ...
            source_data0 = utils.radiation_noise(source_data).float()
            # 1
            source_data1 = utils.flip_augmentation(source_data).float()
            # 2
            target_data0 = utils.radiation_noise(target_data).float()
            # 3
            target_data1 = utils.flip_augmentation(target_data).float()

            source_features, source_proj, source_outputs, target_features, target_proj, target_outputs = feature_encoder(
                source_data.cuda(), target_data.cuda())
            _, source_proj_noise, _, _, target_proj_noise, _ = feature_encoder(
                source_data0.cuda(), target_data0.cuda())
            _, source_proj_flip, _, _, target_proj_flip, _ = feature_encoder(
                source_data1.cuda(), target_data1.cuda())

            softmax_output_t = nn.Softmax(dim=1)(target_outputs).detach()
            _, pseudo_label_t = torch.max(softmax_output_t, 1)

            # Supervised Contrastive Loss
            all_source_con_features = torch.cat([source_proj_noise.unsqueeze(1), source_proj_flip.unsqueeze(1)],dim=1)
            all_target_con_features = torch.cat([target_proj_noise.unsqueeze(1), target_proj_flip.unsqueeze(1)], dim=1)
            ...
            lmmd_loss = mmd.lmmd(source_features, target_features, source_label,
                                 ...
                                 CLASS_NUM=CLASS_NUM)
            lambd = 0.01 * (2 / (1 + math.exp(-10 * (epoch) / epochs)) - 1)
            # Loss Con_s
            ...
            scl_loss = contrastive_loss_t + contrastive_loss_s
            loss = cls_loss + scl_loss + lambd * lmmd_loss
        ...
        if epoch % epochs == 0:
            ...
            with torch.no_grad():
                for test_datas, test_labels in test_loader:
                    ...
                    _, test_outputs = feature_encoder.predict(Variable(test_datas).cuda())

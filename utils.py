...
def radiation_noise(data, alpha_range=(0.9, 1.1), beta=0.04): #pavia/houston = 0.04
    ...
    noise = np.random.normal(loc=0.0, scale=1.0, size=data.shape).astype(np.float32)
    return alpha * data + beta * noise
...
def weak_augmentation(data):
    ...
    if np.random.random() > 0.5:
        augmented = flip_augmentation(augmented)
    return augmented
...
def strong_augmentation(data):
    if torch.is_tensor(data):
        ...
        augmented = np.copy(data)
    augmented = radiation_noise(augmented, beta=0.06)
    augmented = flip_augmentation(augmented)
    if torch.is_tensor(augmented):
        band_dropout = (torch.rand(augmented.size(0), augmented.size(1), 1, 1, device=augmented.device) > 0.1).float()
        augmented = augmented * band_dropout
        return augmented.contiguous()

    band_dropout = (np.random.rand(augmented.shape[0], augmented.shape[1], 1, 1) > 0.1).astype(np.float32)
    return np.ascontiguousarray(augmented * band_dropout)


def target_consistency_augmentation(data):
    if torch.is_tensor(data):
        augmented = data.clone()
    else:
        augmented = np.copy(data)
    augmented = radiation_noise(augmented, beta=0.02)
    augmented = flip_augmentation(augmented)
    if torch.is_tensor(augmented):
        return augmented.contiguous()

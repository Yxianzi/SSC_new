...
class WeightedSupConLoss(SupConLoss):
    """Supervised contrastive loss with sample reliability weights."""
    ...
    def forward(self, features, labels=None, mask=None, sample_weights=None):
        ...
        mean_log_prob_pos[valid_mask] = (mask[valid_mask] * log_prob[valid_mask]).sum(1) / positive_count[valid_mask]

        loss = - (self.temperature / self.base_temperature) * mean_log_prob_pos
        loss = loss.view(anchor_count, batch_size)

        if sample_weights is None:
            sample_weights = torch.ones(batch_size, device=device, dtype=features.dtype)
        else:
            sample_weights = sample_weights.to(device=device, dtype=features.dtype)

        sample_weights = sample_weights.clamp(min=0.0)
        valid_sample_mask = valid_mask.view(anchor_count, batch_size).all(dim=0)
        effective_weights = sample_weights * valid_sample_mask.float()
        normalizer = effective_weights.sum().clamp(min=1.0)

        weighted_loss = (loss.mean(dim=0) * effective_weights).sum() / normalizer
